...
ngo_char<-read.csv("ngo_char.csv")
ngo_char<-ngo_char[3:5]
ava_char<-read.csv("ava_char.csv")
ava_char<-ava_char[3:4]

ngo<-merge(ngo_char, ngo, by="Cal_yrs_BP")
ava<-merge(ava_char, ava, by="Cal_yrs_BP")
...
hist(ava$DIST)
ava_dist<-lm(log1p(DIST) ~ RSL + as.factor(Tephra) + Microcharcoal, data = ava)
summary(ava_dist)
...
hist(ngo$DIST)
ngo_dist<-lm(log1p(DIST) ~ RSL + as.factor(Tephra) + Microcharcoal + Macrocharcoal , data = ngo)
summary(ngo_dist)
...
hist(ava$LITTO)
ava_litto<-lm(log1p(LITTO) ~ RSL + as.factor(Tephra) + Microcharcoal, data = ava)
summary(ava_litto)
...
hist(ngo$LITTO)
ngo_litto<-lm(log1p(LITTO) ~ RSL + as.factor(Tephra) + Microcharcoal + Macrocharcoal, data = ngo)
summary(ngo_litto)
...
hist(ava$MANG)
ava_mang<-lm(log1p(MANG) ~ RSL + as.factor(Tephra) + Microcharcoal, data = ava)
summary(ava_mang)
...
hist(ngo$MANG)
ngo_mang<-lm(log1p(MANG) ~ RSL + as.factor(Tephra) + Microcharcoal + Macrocharcoal, data = ngo)
summary(ngo_mang)
...
#Rainforest####
hist(ava$RAIN)
ava_rain<-lm(log1p(RAIN) ~ RSL + as.factor(Tephra) + Microcharcoal, data = ava)
summary(ava_rain)
...
hist(ngo$RAIN)
ngo_rain<-lm(log1p(RAIN) ~ RSL + as.factor(Tephra) + Microcharcoal + Macrocharcoal, data = ngo)
summary(ngo_rain)
...
hist(ava$WETL)
ava_wetl<-lm(log1p(WETL) ~ RSL + as.factor(Tephra) + Microcharcoal, data = ava)
summary(ava_wetl)
...
hist(ngo$WETL)
ngo_wetl<-lm(log1p(WETL) ~ RSL + as.factor(Tephra) + Microcharcoal + Macrocharcoal, data = ngo)
summary(ngo_wetl)
...
if(1){
  export_summs(ava_dist, ava_litto, ava_mang, ava_rain, ava_wetl,
               ...
               ngo_dist, ngo_litto, ngo_mang, ngo_rain, ngo_wetl, scale = TRUE, to.file = "xlsx", file.name = "LM_results_char.xlsx")}
