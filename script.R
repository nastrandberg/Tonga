...
# get rid of decimal points since they seem to mess up the GLMs later
...
if(1){
  ...
  AVAI_TRENDS<-ggplot(data = AVAI, aes(x = Cal_yrs_BP,  y = Percentage, color = Type)) +
    geom_point() +
    geom_smooth(aes(Cal_yrs_BP,Percentage), se = F) +
...
ava_dist<-glm(DIST ~ RSL + Tephra, family="poisson", data = ava)
summary(ava_dist)

fin_dist<-glm(DIST ~ RSL +Tephra, family="poisson", data = fin)
summary(fin_dist)

lot_dist<-glm(DIST ~ RSL + Tephra, family="poisson", data = lot)
summary(lot_dist)

ngo_dist<-glm(DIST ~ RSL + Tephra, family="poisson", data = ngo)
summary(ngo_dist)

#Littoral####

ava_litto<-glm(DIST ~ RSL + Tephra, family="poisson", data = ava)
summary(ava_litto)

fin_litto<-glm(LITTO ~ RSL + Tephra, family="poisson", data = fin)
summary(fin_litto)

lot_litto<-glm(LITTO ~ RSL + Tephra, family="poisson", data = lot)
summary(lot_litto)

ngo_litto<-glm(LITTO ~ RSL + Tephra, family="poisson", data = ngo)
summary(ngo_litto)

#Mangrove####

ava_mang<-glm(MANG ~ RSL + Tephra, family="poisson", data = ava)
summary(ava_mang)

fin_mang<-glm(MANG ~ RSL + Tephra, family="poisson", data = fin)
summary(fin_mang)

lot_mang<-glm(MANG ~ RSL + Tephra, family="poisson", data = lot)
summary(lot_mang)

ngo_mang<-glm(MANG ~ RSL + Tephra, family="poisson", data = ngo)
summary(ngo_mang)

#Rainforest####

ava_rain<-glm(RAIN ~ RSL + Tephra, family="poisson", data = ava)
summary(ava_rain)

fin_rain<-glm(RAIN ~ RSL + Tephra, family="poisson", data = fin)
summary(fin_rain)

lot_rain<-glm(RAIN ~ RSL + Tephra, family="poisson", data = lot)
summary(lot_rain)

ngo_rain<-glm(RAIN ~ RSL + Tephra, family="poisson", data = ngo)
summary(ngo_rain)

#Wetland####

ava_wetl<-glm(WETL ~ RSL + Tephra, family="poisson", data = ava)
summary(ava_wetl)

fin_wetl<-glm(WETL ~ RSL + Tephra, family="poisson", data = fin)
summary(fin_wetl)

lot_wetl<-glm(WETL ~ RSL + Tephra, family="poisson", data = lot)
summary(lot_wetl)

ngo_wetl<-glm(WETL ~ RSL + Tephra, family="poisson", data = ngo)
summary(ngo_wetl)
...
export_summs(ava_dist, ava_litto, ava_mang, ava_rain, ava_wetl,
               ...
               ngo_dist, ngo_litto, ngo_mang, ngo_rain, ngo_wetl, scale = TRUE, to.file = "xlsx", file.name = "GLM_results.xlsx")

# for some reason the "lot_wetl" summary won't plot with the others